...
export const buttonVariants = tv({
  base: "relative inline-flex cursor-default select-none items-center justify-center gap-2 rounded-full font-medium text-sm outline-offset-4 transition focus-visible:outline-4 disabled:pointer-events-none disabled:opacity-50",
  variants: {
    size: {
  ...
  compoundVariants: [
    ...
    },
    {
      ...
      tone: "neutral",
      class:
        "[--btn-bg:theme(colors.surface)] hover:bg-on-surface/5 focus:bg-on-surface/8",
    },
    {
      variant: "ghost",
      tone: "primary",
    ...
    },
    {
      ...
      tone: "destructive",
      class:
        "text-error [--btn-bg:theme(colors.error/0.05)] hover:bg-error/5 focus:bg-error/8",
    },
    {
      variant: "link",
...
const SplitButton = (
  ownProps: Omit<BaseProps<"div">, "component" | "icon">
) => {
  return (
    <BaseComponent
      {...ownProps}
